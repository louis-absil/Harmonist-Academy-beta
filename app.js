...
export const App = {
    ...
    handleSprintFail() {
        ...
        let invName = "";
        if (this.data.currentSet === 'laboratory') {
            invName = c.type.configs[c.inv].name;
        } else if (!isDim) {
            invName = DB.currentInvs[c.inv].name;
        }
    ...
    preview(typeStr, id) {
        ...
        let targetType = DB.chords.find(c => c.id === targetTypeId);
    ...
    select(type, id) {
        ...
        if(type === 'c') {
            ...
            this.session.selectionHistory.push(id);
    ...
    processWin(okC, okI) {
        ...
        if(!isTrivial) {
            if(this.data.currentSet === 'jazz') {
                 ...
                 if(okI) this.data.stats.l[lKey].ok++;
            } else {
                 if(!isDim) {
        ...
        if(win) {
            ...
            if(!isTrivial) {
                ...
                else if(this.data.currentSet === 'laboratory') newInvObj = this.data.stats.l[`${c.type.id}_${c.inv}`];
                ...
                if(c.type.id === 'trichord') this.session.triStreak++; else this.session.triStreak = 0;
                
                if(this.data.currentSet === 'jazz' && c.inv === 3) this.session.rootlessStreak++; else this.session.rootlessStreak = 0;
                if(this.data.settings.activeC.length === 1) this.session.monoStreak++; else this.session.monoStreak = 0;
            ...
            this.session.str36Streak = 0; this.session.str45Streak = 0; 
            ...
            let invName = "";
            if(this.data.currentSet === 'laboratory') invName = c.type.configs[c.inv].name;
            else if(!isDim) invName = DB.currentInvs[c.inv].name;
    ...
    cheat(scenario) {
        ...
        switch(scenario) {
            case 'weak_inv':
                d.stats.totalPlayed = 200; 
                d.stats.c = {}; d.stats.i = {};
                DB.chords.forEach(c => d.stats.c[c.id] = {ok:20, tot:20}); 
                DB.invs.forEach(i => {
            ...
            case 'endgame':
                ...
                break;
            case 'badges':
                d.badges = BADGES.map(b => b.id).filter(id => id !== 'b_leg'); 
