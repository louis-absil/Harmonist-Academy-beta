...
export const UI = {
    ...
    getLoreState(m) {
        ...
        // Construction du Nom Complet (Grammaire Automatique)
    ...
    // --- BADGE LIGHTBOX (GLASSMORPHISM & CLICK-THROUGH) ---
    openBadgeLightbox(b) {
        ...
        if (!el) {
            ...
            // CLICK-THROUGH LOGIC: Close on ANY click
            el.onclick = () => {
    ...
    renderProfile() {
        ...
        // VISUAL REWORK: TECH > LORE
        // Swap hierarchy: 
        // profileMasteryName becomes the Main Tech Row (M-XX + Stars)
        // profileStars (repurposed) becomes CLEARED to avoid clutter in header

        const techContainer = document.getElementById('profileMasteryName');
        const loreContainer = document.getElementById('profileStars');

        // Main Tech Row content
        ...
        // Clear the old location in header
        ...
        // --- NEW: FOOTER LORE (FOOTNOTE) ---
        // On crée ou cible un footer technique en bas de la modale pour le titre "Fantasy"
        let footerLore = document.getElementById('profileFooterLore');
        if(!footerLore) {
            footerLore = document.createElement('div');
            footerLore.id = 'profileFooterLore';
            footerLore.className = 'profile-footer-lore';
            // Insert at the end of the modal content
            document.querySelector('#modalProfile .modal').appendChild(footerLore);
        }
    ...
    renderStats() {
        ...
        // --- BADGES RENDER V2 (CATEGORIES & SECRETS & SUB-SETS) ---
        ...
        categories.forEach(cat => {
            ...
            if (visibleBadges.length > 0) {
                ...
                // SUB-GROUPING LOGIC FOR SPECIALISATIONS
                if(cat.id === 'special') {
                    // Group by setID
                    const groups = { general: [] };
                    visibleBadges.forEach(b => {
                        const sid = b.setID || 'general';
                        if(!groups[sid]) groups[sid] = [];
                        groups[sid].push(b);
                    });

                    // Define Order: General first, then DB.sets order
                    const setKeys = Object.keys(DB.sets);
                    const renderOrder = ['general', ...setKeys];

                    renderOrder.forEach(sid => {
                        if(groups[sid] && groups[sid].length > 0) {
                            // If it's a named set, render a sub-header
                            if(sid !== 'general') {
                                const setName = DB.sets[sid] ? DB.sets[sid].name.toUpperCase() : sid.toUpperCase();
                                const subHeader = document.createElement('div');
                                subHeader.style.cssText = "grid-column: 1 / -1; font-size: 0.65rem; color: var(--text-dim); text-align:center; margin: 10px 0 5px 0; letter-spacing:1px; display:flex; align-items:center; justify-content:center; gap:10px;";
                                subHeader.innerHTML = `<span style="height:1px; background:rgba(255,255,255,0.1); flex:1;"></span><span>${setName}</span><span style="height:1px; background:rgba(255,255,255,0.1); flex:1;"></span>`;
                                grid.appendChild(subHeader);
                            }
                            // Render badges in this group
                            groups[sid].forEach(b => renderBadge(b));
                        }
                    });

                } else {
                    // Standard rendering for General and Lore
                    visibleBadges.forEach(b => renderBadge(b));
                }
            }
        });
        
        // Hide Old Footer Detail
